...
def header_image(request):
    url = cache.get('HEADER_IMAGE_URL')
    if url is None:
        si = SiteImage.objects.filter(name="Header Image").first()
        url = si.image.url if si and si.image else static('engfroshHeader.png')
        cache.set('HEADER_IMAGE_URL', url, 300)
...
def favicon32(request):
    ...
    if url is None:
        ...
        url = si.image.url if si and si.image else static('favicon-32x32.ico')
...
def favicon16(request):
    ...
    if url is None:
        ...
        url = si.image.url if si and si.image else static('favicon-16x16.ico')
